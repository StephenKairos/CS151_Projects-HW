...
public class TileGrid extends Observable implements Iterable<JLabel>,
		Iterator<JLabel> {
	 ...
	 * 
	 * @param image
	 *            BufferedImage to be broken into tiles
	 * @param rows
	 *            number of rows
	 * @param columns
	 *            number of columns
	...
	public TileGrid(BufferedImage image, int rows, int columns) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < columns; j++) {
				BufferedImage tileImage = puzzleImage.getSubimage(j
						* puzzleImageWidth / columns, i * puzzleImageHeight
						/ rows, puzzleImageWidth / columns, puzzleImageHeight
						/ rows);
	 ...
	 * @return true if the tile can be moved in the given direction. False if
	 *         not.
	 */
	...
	public boolean canMoveRight(int row, int column) {
		if ((column + 1) < columns
				&& tileGrid.get(row).get(column + 1) == blankTile)
			return true;
		return false;
	}
	 ...
	 * 
	 * @param rowFrom
	 *            the row to be moved from
	 * @param columnFrom
	 *            the column to be moved from
	 * @param rowTo
	 *            the row to be moved to
	 * @param columnTo
	 *            the column to be moved to
	 */
	...
	/**
	 * Our iterator override. This allows for tiles to be returned in the order
	 * they should be drawn in for GridLayout.
	 */
	@Override
	...
	/**
	 * Method to be called whenever data in tileGrid is modified. This will
	 ...
	 * 
	 * @param iterations
	 *            how many times the blank tile should be moved
	 */
